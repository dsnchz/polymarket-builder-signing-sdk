...
describe("builder config", () => {
  ...
  it("getBuilderType", async () => {
    ...
    builderConfig = new BuilderConfig({ localBuilderCreds: creds });
    expect(builderConfig.getBuilderType()).toEqual(BuilderType.LOCAL);

    builderConfig = new BuilderConfig({
      remoteBuilderConfig: { url: "http://localhost:3000/sign" },
    });
    expect(builderConfig.getBuilderType()).toEqual(BuilderType.REMOTE);

    builderConfig = new BuilderConfig();
    expect(builderConfig.getBuilderType()).toEqual(BuilderType.UNAVAILABLE);
  ...
  it("generateBuilderHeaders", async () => {
    ...
    expect(headers.POLY_BUILDER_API_KEY).toEqual("019894b9-cb40-79c4-b2bd-6aecb6f8c6c5");
    expect(headers.POLY_BUILDER_PASSPHRASE).toEqual(
      "1816e5ed89518467ffa78c65a2d6a62d240f6fd6d159cba7b2c4dc510800f75a",
    );
    expect(headers.POLY_BUILDER_TIMESTAMP).toEqual("1758744060");
    expect(headers.POLY_BUILDER_SIGNATURE).toEqual("8xh8d0qZHhBcLLYbsKNeiOW3Z0W2N5yNEq1kCVMe5QE=");
  });
  ...
  it("generateHeaders - remote", async () => {
    ...
    expect(headers!.POLY_BUILDER_API_KEY).toEqual("test-api-key");
    expect(headers!.POLY_BUILDER_TIMESTAMP).toEqual("1758744060");
    expect(headers!.POLY_BUILDER_PASSPHRASE).toEqual("test-passphrase");
    expect(headers!.POLY_BUILDER_SIGNATURE).toEqual("test-signature");
  });
});
